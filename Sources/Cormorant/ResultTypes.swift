...
enum EvalResult {
  ...
  func then(_ next: (Value) -> EvalResult) -> EvalResult {
    switch self {
...
public enum EvalOptional<T> {
  ...
  func then(_ next: (T) -> EvalResult) -> EvalResult {
    switch self {
...
enum ReadOptional<T> {
  ...
  func then<U>(_ next: (T) -> ReadOptional<U>) -> ReadOptional<U> {
    switch self {
    case let .Just(s): return next(s)
  ...
  func then(_ fn: String, _ next: (T) -> EvalResult) -> EvalResult {
    switch self {
    case let .Just(s): return next(s)
