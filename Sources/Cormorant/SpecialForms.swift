...
internal var QUOTE : Value { Value.special(.Quote) }
internal var IF : Value { Value.special(.If) }
internal var DO : Value { Value.special(.Do) }
internal var DEF : Value { Value.special(.Def) }
internal var LET : Value { Value.special(.Let) }
internal var VAR : Value { Value.special(.Var) }
internal var FN : Value { Value.special(.Fn) }
internal var DEFMACRO : Value { Value.special(.Defmacro) }
internal var LOOP : Value { Value.special(.Loop) }
internal var RECUR : Value { Value.special(.Recur) }
internal var APPLY : Value { Value.special(.Apply) }
...
func sf_def(_ args: Params, _ ctx: Context) -> EvalResult {
  ...
  switch symbol {
  case let .symbol(sym):
    if let ns = sym.ns, ns != ctx.interpreter.currentNsName {
...
private func do_exprs<T: Collection>(_ args: T, _ ctx: Context) -> EvalResult where T.Iterator.Element == Value, T.Index == Int {
  let fn = "do"
