...
func stringData(forBundleFileNamed name: String) -> String? {
  let candidateBundles: [Bundle] = [
    .module,
    Bundle(for: Interpreter.self),
    .main
  ]

  var path: String? = nil
  for bundle in candidateBundles {
    if let rootPath = bundle.path(forResource: name, ofType: "lbt") {
      path = rootPath
      break
    }
    if let stdlibPath = bundle.path(forResource: name, ofType: "lbt", inDirectory: "stdlib") {
      path = stdlibPath
      break
    }
  }
