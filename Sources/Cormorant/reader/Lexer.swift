...
private struct Lexer {
  ...
  static func lex2(rawTokenBuffer: [RawLexToken]) -> ReadOptional<[LexToken]> {
    ...
    for rawToken in rawTokenBuffer {
      ...
      case let .unknown(unknown):
        ...
        else if unknown.first == ":" {
          // This is a keyword (starts with ":" and has at least one other character)
          tokenBuffer.append(.keyword(String(unknown[unknown.index(after: unknown.startIndex)..<unknown.endIndex])))
        }
        else if unknown == "nil" {
          // Literal nil
  ...
  static func consumeComment(in str: String, index: inout String.Index) {
    var current = index

    while current < str.endIndex {
      let c = str[current]
      str.formIndex(after: &current)
      if character(c, isMemberOfSet: CharacterSet.newlines) {
  ...
  static func consumeStringLiteral(in str: String, index: inout String.Index, raw: Bool) -> ReadOptional<String> {
  ...
  static func consumeTilde(in str: String, index: inout String.Index) -> SyntaxToken {
  ...
  static func consumeHash(in str: String, index: inout String.Index) -> ReadOptional<RawLexToken> {
    // Precondition: str[index] must be '#'.
    if index == str.indexOfLastCharacter {
  ...
  static func consumeCharacter(in str: String, index: inout String.Index) -> ReadOptional<Character> {
    ...
    if firstCharacter == "u"  {
      ...
      if let d0 = digitAsNumber(str, &thisIndex, .Hexadecimal),
         let d1 = digitAsNumber(str, &thisIndex, .Hexadecimal),
         let d2 = digitAsNumber(str, &thisIndex, .Hexadecimal),
         let d3 = digitAsNumber(str, &thisIndex, .Hexadecimal),
         thisIndex == str.endIndex || characterTerminatesLiteral(str[thisIndex]) {
          index = thisIndex
          let value : Int = 4096*d0 + 256*d1 + 16*d2 + d3
          if let scalar = UnicodeScalar(value) {
            return .Just(Character(scalar))
          }
    ...
    if firstCharacter == "o" {
      ...
      if let d0 = digitAsNumber(str, &thisIndex, .Octal),
         let d1 = digitAsNumber(str, &thisIndex, .Octal),
         let d2 = digitAsNumber(str, &thisIndex, .Octal),
         thisIndex == str.endIndex || characterTerminatesLiteral(str[thisIndex]) {
          index = thisIndex
          let value : Int = 64*d0 + 8*d1 + d2
          if value < 256, let scalar = UnicodeScalar(value) { return .Just(Character(scalar)) }
      }
...
private func digitAsNumber(_ string: String, _ idx: inout String.Index, _ type: NumberType) -> Int? {
