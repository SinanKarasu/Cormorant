...
func internalError(_ message: @autoclosure () -> String) -> Never {
...
struct PairSequenceGenerator<T : Sequence> : IteratorProtocol {
  ...
  fileprivate init(_ seq: T) { self.seq = seq; g = seq.makeIterator() }
...
func character(_ c: Character, isMemberOfSet set: CharacterSet) -> Bool {
  return String(c).unicodeScalars.allSatisfy { set.contains($0) }
}
...
func characterAtIndex(_ s: String, idx: Int) -> Character? {
  for (stringIdx, character) in s.enumerated() {
    if stringIdx == idx {
      return character
    }
  }
...
internal extension String {
  ...
  func character(after theIndex: String.Index) -> Character {
    ...
    return self[self.index(after: theIndex)]
  }
