...
/// A protocol that types can conform to if they wish to be accessible as Cormorant objects. All values of
/// AuxiliaryType are atoms which evaluate to themselves.
public protocol AuxiliaryType: AnyObject {
  // Unfortunately, given Swift's lack of support for introspection/reflection, the use of AuxiliaryType to add support
  //  for arbitrary Swift types to Cormorant at runtime is mostly circumscribed. This protocol is meant for making it
  //  easier to add features to Cormorant that require runtime support.
...
public final class StringBuilderType : AuxiliaryType {
  ...
  func append(_ str: String) { buffer += Array(str) }
  ...
  init() { buffer = [] }
  init(_ str: String) { buffer = Array(str) }
}
