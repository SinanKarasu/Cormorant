...
public typealias LoggingFunction = (@escaping () -> String) -> Void
...
/// A class representing a Cormorant interpreter.
public class Interpreter {
  ...
  /// The currently active namespace. There must always be an active namespace after initialization.
  private(set) var currentNamespace : NamespaceContext! {
    willSet {
      // Namespace is being changed; change *ns* as well
      if let core = coreNamespace, let newValue = newValue {
  ...
  public func setLoggingFunction(domain: LogDomain, function: @escaping LoggingFunction) {
    switch domain {
    case .Eval:
  ...
  /// Given a string, evaluate it as Cormorant code and return a successful result or error.
  ...
  /// Given a Cormorant form, return a prettified description.
  ...
  func log(_ domain: LogDomain, _ message: @escaping () -> String) {
    switch domain {
    case .Eval:
      evalLogging?(message)
    }
  ...
  func remove(namespace ns: NamespaceName) -> NamespaceResult {
    if let namespaceToDelete = namespaces[ns], namespaceToDelete.isSystemNamespace {
      return .Error(EvalError(.ReservedNamespaceError))
    }
  ...
  public init() {
    ...
    #if DEBUG
      print("Cormorant interpreter: framework was built using DEBUG mode")
    #endif
  }
}
