...
final public class NamespaceContext : Context, Hashable {
  ...
  func alias(namespace: NamespaceContext, usingAlias a: NamespaceName) -> EvalError? {
    ...
    if let existingNamespace = aliases[a], !(namespace === existingNamespace) {
  ...
  func resolveVar(for symbol: InternedSymbol) -> VarType? {
    if let ns = symbol.ns, ns != internedName {
  ...
  func refer(_ namespace: NamespaceContext) -> EvalError? {
    ...
    for (symbol, aVar) in namespace.vars {
      ...
      if let otherRefer = refers[unqualified], otherRefer.name.ns != namespace.internedName {
...
//  }
  ...
  func setVar(named varName: UnqualifiedSymbol, toValue newValue: Value) -> EvalOptional<VarType> {
    if let alreadyReferred = refers[varName] {
      ...
      if let ns = alreadyReferred.name.ns, interpreter.namespaces[ns]?.isSystemNamespace == true {
...
final class LexicalScopeContext : Context {
  ...
  private func findBinding(for symbol: UnqualifiedSymbol) -> Value? {
    if let b0 = b0, b0.symbol == symbol {
      return b0.binding
    }
    if let b1 = b1, b1.symbol == symbol {
      return b1.binding
    }
    if let b2 = b2, b2.symbol == symbol {
      return b2.binding
    }
    if let b3 = b3, b3.symbol == symbol {
      return b3.binding
    }
    if let b4 = b4, b4.symbol == symbol {
      return b4.binding
    }
    if let b5 = b5, b5.symbol == symbol {
      return b5.binding
    }
    if let b6 = b6, b6.symbol == symbol {
      return b6.binding
    }
    if let b7 = b7, b7.symbol == symbol {
      return b7.binding
    }
    if let b8 = b8, b8.symbol == symbol {
      return b8.binding
    }
    if let b9 = b9, b9.symbol == symbol {
      return b9.binding
    }
    if let b10 = b10, b10.symbol == symbol {
      return b10.binding
    }
    if let b11 = b11, b11.symbol == symbol {
      return b11.binding
    }
    if let b12 = b12, b12.symbol == symbol {
      return b12.binding
    }
    if let b13 = b13, b13.symbol == symbol {
      return b13.binding
    }
    if let b14 = b14, b14.symbol == symbol {
      return b14.binding
    }
    if let b15 = b15, b15.symbol == symbol {
      return b15.binding
    }
