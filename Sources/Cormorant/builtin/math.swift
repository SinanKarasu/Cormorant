...
func pr_plus(args: Params, _ ctx: Context) -> EvalResult {
  ...
  case let .integer(v1):
    switch num1 {
    case let .integer(v2):
      let (sum, overflow) = v1.addingReportingOverflow(v2)
...
func pr_minus(args: Params, _ ctx: Context) -> EvalResult {
  ...
  case let .integer(v1):
    switch num1 {
    case let .integer(v2):
      let (difference, overflow) = v1.subtractingReportingOverflow(v2)
...
func pr_multiply(args: Params, _ ctx: Context) -> EvalResult {
  ...
  case let .integer(v1):
    switch num1 {
    case let .integer(v2):
      let (product, overflow) = v1.multipliedReportingOverflow(by: v2)
...
func pr_divide(args: Params, _ ctx: Context) -> EvalResult {
  ...
  case let .integer(v1):
    ...
    case let .integer(v2):
      ...
      let (remainder, overflow) = v1.remainderReportingOverflow(dividingBy: v2)
      if overflow {
        return .Failure(EvalError(.IntegerOverflowError, fn))
      }
      else if remainder == 0 {
        let (quotient, overflow) = v1.dividedReportingOverflow(by: v2)
