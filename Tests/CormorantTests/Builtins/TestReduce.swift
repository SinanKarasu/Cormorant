...
//  }
  ...
  func testWithValOnMap() {
    ...
    let b = keyword("b")
    run(input: "(def tf (fn [a b] (.print \"a:\" a \"b:\" b) (.concat a b)))")
    expectThat("(.reduce tf [6 7 8 9] {:a 1 :b 2})",
               shouldEvalToContain: 6, 7, 8, 9,
               .keyword(a), 1, .keyword(b), 2)
    let validOutputs: Set<String> = [
      "a: [6 7 8 9] b: [:b 2]a: (6 7 8 9 :b 2) b: [:a 1]",
      "a: [6 7 8 9] b: [:a 1]a: (6 7 8 9 :a 1) b: [:b 2]"
    ]
    XCTAssert(validOutputs.contains(outputBuffer),
              "Unexpected output buffer order for map reduce: \(outputBuffer)")
  }
