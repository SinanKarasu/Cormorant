...
class TestSyntaxQuoteAutoGensym : InterpreterTest {
  ...
  func testMaps() {
    ...
    switch lex(input) {
    case let .Just(lexed):
      switch parse(tokens: lexed, context) {
      case let .Just(parsed):
        switch context.expand(parsed) {
        case let .Success(expanded):
          let actual: String
          switch expanded.describe(context) {
          case let .Just(value):
            actual = value
          case let .Error(err):
            XCTFail("description error: \(err.description)")
            return
          }

          XCTAssert(actual.hasPrefix("(apply .hashmap"),
                    "Expected syntax-quoted map expansion, got: \(actual)")

          func matches(for symbol: String) -> [String] {
            let pattern = "\(symbol)__[0-9]+__auto__"
            guard let regex = try? NSRegularExpression(pattern: pattern) else {
              return []
            }
            let range = NSRange(actual.startIndex..<actual.endIndex, in: actual)
            return regex.matches(in: actual, range: range).compactMap {
              Range($0.range, in: actual).map { String(actual[$0]) }
            }
          }

          let aMatches = matches(for: "a")
          let bMatches = matches(for: "b")
          let cMatches = matches(for: "c")
          let dMatches = matches(for: "d")

          XCTAssert(aMatches.count == 2 && Set(aMatches).count == 1,
                    "a# should map to one gensym reused twice; got \(aMatches)")
          XCTAssert(bMatches.count == 2 && Set(bMatches).count == 1,
                    "b# should map to one gensym reused twice; got \(bMatches)")
          XCTAssert(cMatches.count == 1 && Set(cMatches).count == 1,
                    "c# should map to one gensym; got \(cMatches)")
          XCTAssert(dMatches.count == 1 && Set(dMatches).count == 1,
                    "d# should map to one gensym; got \(dMatches)")
        case let .Failure(f):
