...
class TestDescribing : InterpreterTest {
  ...
  lazy var rawStrings : [String] = {
    if let path = Bundle.module.path(forResource: "rawstrings", ofType: "txt", inDirectory: "SupportingFiles"),
       let contents = try? String(contentsOfFile: path, encoding: .utf8) {
      return contents.split(whereSeparator: \.isNewline).map(String.init)
    }
    fatalError("ERROR! Could not load strings from test support file rawstrings.txt")
  }()
