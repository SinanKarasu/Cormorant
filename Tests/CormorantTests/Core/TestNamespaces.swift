...
class TestNamespaceRefer : InterpreterTest {
  ...
  func testInternAfterReferSystemNamespace() {
    ...
    systemNamespace.setVar(named: symbol("a"), toValue: 1000)
    interpreter.testOnly_add(namespace: systemNamespace, named: sys)
    // Refer the namespace
    run(input: "(.ns-refer 'sys)")
    expectThat("a", shouldEvalTo: 1000)
    // Try interning a local Var
    run(input: "(def a \"foobar\")")
    expectThat("a", shouldEvalTo: .string("foobar"))
  ...
  func testReferNameConflict() {
    // Build a namespace
    run(input: "(.ns-set 'ns1)")
    run(input: "(def a 10)")
    run(input: "(.ns-set 'ns2)")
    run(input: "(def a 20)")
    run(input: "(.ns-set 'user)")
    // Refer ns1
    run(input: "(.ns-refer 'ns1)")
  ...
  func testReferredVarRebinding() {
    ...
    run(input: "(.ns-set 'firstNamespace)")
    run(input: "(.ns-refer 'toRefer)")
    expectThat("a", shouldEvalTo: 10)
    // Try it in another namespace
    run(input: "(.ns-set 'secondNamespace)")
    run(input: "(.ns-refer 'toRefer)")
    expectThat("a", shouldEvalTo: 10)
...
class TestSymbolNamespacing : InterpreterTest {
  ...
  func testVarBoundToQualifiedSymbol() {
    expectThat("myVar", shouldFailAs: .InvalidSymbolError)
    run(input: "(def user/myVar 12345)")
    expectThat("myVar", shouldEvalTo: 12345)
    expectThat("user/myVar", shouldEvalTo: 12345)
  }
  ...
  func testSameNamedSymbols() {
    ...
    // Define in 'baz'
    run(input: "(.ns-set 'baz)")
    expectThat("a", shouldFailAs: .InvalidSymbolError)
    run(input: "(def a -1200)")
